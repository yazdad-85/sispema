        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Edit Struktur Biaya</h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('fee-structures.update', $feeStructure->id) }}" method="POST">
                                        ...
                                        <select class="form-control @error('institution_id') is-invalid @enderror" 
                                            ...
                                            @foreach($institutions as $institution)
                                                <option value="{{ $institution->id }}" 
                                                        {{ old('institution_id', $feeStructure->institution_id) == $institution->id ? 'selected' : '' }}>
                                                    {{ $institution->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    @else
                                        <input type="hidden" name="institution_id" value="{{ $feeStructure->institution_id }}">
                                        <input type="text" class="form-control" value="{{ $feeStructure->institution->name ?? '-' }}" readonly>
                                        <small class="form-text text-muted">Lembaga mengikuti data struktur biaya ini</small>
                                    @endif
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('level') is-invalid @enderror" 
                                        ...
                                        <option value="VII" {{ old('level', $feeStructure->class->safe_level ?? '') == 'VII' ? 'selected' : '' }}>VII - Tingkat 1 (SMP/MTs)</option>
                                        <option value="VIII" {{ old('level', $feeStructure->class->safe_level ?? '') == 'VIII' ? 'selected' : '' }}>VIII - Tingkat 2 (SMP/MTs)</option>
                                        <option value="IX" {{ old('level', $feeStructure->class->safe_level ?? '') == 'IX' ? 'selected' : '' }}>IX - Tingkat 3 (SMP/MTs)</option>
                                        <option value="X" {{ old('level', $feeStructure->class->safe_level ?? '') == 'X' ? 'selected' : '' }}>X - Tingkat 1 (SMA/MA/SMK)</option>
                                        <option value="XI" {{ old('level', $feeStructure->class->safe_level ?? '') == 'XI' ? 'selected' : '' }}>XI - Tingkat 2 (SMA/MA/SMK)</option>
                                        <option value="XII" {{ old('level', $feeStructure->class->safe_level ?? '') == 'XII' ? 'selected' : '' }}>XII - Tingkat 3 (SMA/MA/SMK)</option>
                                    </select>
                                    ...
                                    <select class="form-control @error('academic_year_id') is-invalid @enderror" 
                                        ...
                                        @foreach($academicYears as $academicYear)
                                            <option value="{{ $academicYear->id }}" 
                                                    {{ old('academic_year_id', $feeStructure->academic_year_id) == $academicYear->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('scholarship_discount') is-invalid @enderror" 
                                           ...
                                           value="{{ old('scholarship_discount', $feeStructure->scholarship_discount) }}" 
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('monthly_amount') is-invalid @enderror" 
                                           ...
                                           value="{{ old('monthly_amount', $feeStructure->monthly_amount) }}" min="0" 
                                           placeholder="500000">
                                    <small class="form-text text-muted">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('yearly_amount') is-invalid @enderror" 
                                           ...
                                           value="{{ old('yearly_amount', $feeStructure->yearly_amount) }}" min="0" 
                                           placeholder="6000000">
                                    <small class="form-text text-muted">
                        ...
                        <input type="hidden" id="class_id" name="class_id" value="{{ old('class_id', $feeStructure->class_id) }}">
                        ...
                        <div class="form-group">
                            ...
                            <a href="{{ route('fee-structures.show', $feeStructure->id) }}" class="btn btn-secondary">
...
// Dynamic class loading based on selected institution (for admin pusat only)
// Auto-select class based on level and institution
document.getElementById('level').addEventListener('change', function() {
    const level = this.value;
    const institutionId = document.getElementById('institution_id').value || '{{ $feeStructure->institution_id }}';
...
// Clear level and class when institution changes (for admin pusat only)
@if(Auth::user()->role === 'admin_pusat')
