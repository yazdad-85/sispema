        ...
        <div class="card-body">
            <form action="{{ route('classes.update', $class->id) }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('grade_level') is-invalid @enderror" 
                                ...
                                <option value="MI" {{ old('grade_level', $class->grade_level) == 'MI' ? 'selected' : '' }}>MI (Madrasah Ibtidaiyah)</option>
                                <option value="SD" {{ old('grade_level', $class->grade_level) == 'SD' ? 'selected' : '' }}>SD (Sekolah Dasar)</option>
                                <option value="MTs" {{ old('grade_level', $class->grade_level) == 'MTs' ? 'selected' : '' }}>MTs (Madrasah Tsanawiyah)</option>
                                <option value="SMP" {{ old('grade_level', $class->grade_level) == 'SMP' ? 'selected' : '' }}>SMP (SMP)</option>
                                <option value="MA" {{ old('grade_level', $class->grade_level) == 'MA' ? 'selected' : '' }}>MA (Madrasah Aliyah)</option>
                                <option value="SMA" {{ old('grade_level', $class->grade_level) == 'SMA' ? 'selected' : '' }}>SMA (SMA)</option>
                                <option value="SMK" {{ old('grade_level', $class->grade_level) == 'SMK' ? 'selected' : '' }}>SMK (SMK)</option>
                            </select>
                            @error('grade_level')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="level">Kelas <span class="text-danger">*</span></label>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('institution_id') is-invalid @enderror" 
                                ...
                                @foreach($institutions as $institution)
                                    <option value="{{ $institution->id }}" {{ old('institution_id', $class->institution_id) == $institution->id ? 'selected' : '' }}>
                                        {{ $institution->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('capacity') is-invalid @enderror" 
                                   id="capacity" name="capacity" value="{{ old('capacity', $class->capacity) }}" min="1" required>
                            @error('capacity')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
function updateClassOptions() {
    ...
    // Reset options
    levelSelect.innerHTML = '<option value="">Pilih Kelas</option>';
    ...
    if (gradeLevel === 'MI' || gradeLevel === 'SD') {
        ...
        levels.forEach(level => {
            ...
            option.value = level;
            option.textContent = `Kelas ${level}`;
...
// Set initial class options and select current class
document.addEventListener('DOMContentLoaded', function() {
    const gradeLevel = document.getElementById('grade_level').value;
    if (gradeLevel) {
        updateClassOptions();
        // Set current level if it exists
        const currentLevel = '{{ $class->level }}';
        if (currentLevel) {
            const levelSelect = document.getElementById('level');
            // Find and select the current level
